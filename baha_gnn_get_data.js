...
`烈火戰馬||Iron Conflict`,
`帝國神話||Myth of Empires`,
`實況！救援大作戰||Rescue Party: Live!`,
`獵魔者Demon Hunter：覺醒`,
`主公在上我在下||殿は私の上で`,
`斜槓勇者||スラッシュ ユウシャ||Slash Brave`,
`女神都市`,
`代號：紅月||Hunters’ Moon`,
`代號：夜鶯`,
`斯露德||THRUD`,
`代號：了不起的模擬器`,
`代號：艾塔`,
`伊蘇：夢境交織的長夜||Lacrimosa of DANA`,
`寶石研物語：伊恩之石||GemTrader`,
`代號：依露希爾`,
`代號 C`,
`無盡奧德賽`,
`終末陣線：伊諾貝塔||Final Front: Enobetta`,
`悠久之樹||Eternal Tree`,
`太吾繪卷 ||The Scroll of Taiwu`,
`工匠與旅人||Craftsman and Traveler`,
...
`星魂2M`,
`卡里古拉 2||Caligula 2`,
`伊蘇 IX -怪人之夜-||Ys IX: Monstrum Nox`,
`伊蘇 塞爾塞塔的樹海||Ys: Memories of Celceta`,
`英雄傳說 零之軌跡：改||The Legend of Heroes - Zero no Kiseki`,
`英雄傳說 碧之軌跡：改||The Legend of Heroes: Trails of Blue Flame`,
`方根膠捲||Root Film`,
`英雄傳說 創之軌跡||The Legend of Heroes: Hajimari no Kiseki`,
`美俏女劍士 起源|| 美俏女劍士ORIGIN`,
`魔女之泉3 Re:Fine`,
`坦克戰記 異傳：重生||METAL MAX Xeno Reborn`,
`童話森林||Märchen Forest`,
`英雄傳說 閃之軌跡 I：改`,
`英雄傳說 閃之軌跡II：改`,
`搖曳露營△ VIRTUAL CAMP||Laid-Back Camp - Virtual -`,
`英雄傳說 閃之軌跡III (PC)`,
`秋葉脫物語：Hellbound & Debriefed||Akiba's Trip: Undead & Undressed`,
`Relayer ||Relayer `,
`英雄傳說 黎之軌跡||The Legend of Heroes: Kuro no Kiseki`,
`英雄傳說 閃之軌跡IV -THE END OF SAGA-||The Legend of Heroes: Sen no Kiseki IV -THE END OF SAGA-`,
`魔眼凝望 EXTRA||Demon Gaze EXTRA`,
`罪惡王權||Monark`,
`七號堡壘||last fortress:Underground`,
`命運契約||Fate Contract`,
`滄月戰紀 || 滄月：幻境連結||Final Fate TD`,
`來自星塵 || エクスアストリス||Ex Astris`,
`盛世君王 || 帝王生涯`,
`緋紅戰線 ||Diverse Order`,
`伊格效應||Ego Effect`,
`因狄斯的謊言||Indies' Lies`,
...
`天命奇御二||Fate Seeker 2`,
`流火之詩 II：燃||Eternal Scarlet II`,
`無盡征途||Ever Legion`,
`斗羅大陸 - 鬥神再臨||Doula Continent: Reunion`,
`大俠有點窮`,
`夢幻島傳說||The Legend of Neverland`,
`魔女兵器||Witch Weapon`,
`劍開山門`,
`唱舞全明星`,
`尖塔奇兵||Slay the Spire || 殺戮尖塔`,
`遇見妳的貓||Purrfect Tale`,
`冒險大亂鬥-童話的逆襲`,
`三國志天下布武||Rise Of Three Kingdoms`,
`喵咪鬥惡龍||Cat Quest `,
`武器店物語||Weapon Shop Fantasy`,
`TREN0||Tren0`,
`奮鬥吧領主大人||Tiny-Tasy Town`,
`legioncraft`,
`宇宙大拍扁||Big Bia`,
`Tap Titans`,
`吸血鬼戀人||Immortal Diaries`,
`星動時分`,
`大當家：財源廣進||Merchant Master`,
`主公的遠征`,
`魚樂大亨 || 千炮捕魚`,
`挖掘機我最強||Stone Miner`,
`菲菲大冒險 || ファミリーファームの冒険||family farm adventure`,
`Z紀元 || エイジオブゼット||age of z origins`,
`ARKA-蒼穹之門 || ARKA-蒼穹の門`,
`奇蹟之劍 || 魔剣伝説`,
`ウッディブロック：エンドレスパズル||Woody Block：Endless Puzzle Game`,
`成名錄～華與武之戰國 || 成り上がり　華と武の戦国`,
`大老闆 || おねがい社長！||big boss`,
`日替わり内室`,
`戦艦帝国-228艘の実在戦艦を集めろ`,
`叫我萬歲爺 || アイアム皇帝`,
`騎士的公主養成`,
`伊甸園的驕傲 || レッド：プライドオブエデン||Red: Pride of Eden`,
`貓咪軍團大作戰`,
`決勝時刻`,
`一念逍遙`,
`偌星 ||Starlike`,
...
`蔚藍檔案`,
`鐵甲雄兵||Blood of Steel`,
`生死狙擊||battle teams || Blood Strike || Global Strike`,
`音樂少女||Music Girls`,
`世上英雄`,
`守望傳說 ||ガーディアンテイルズ|| Guardian Tales`,
`魔物調查者||Tiny Pixel Knight`,
`魔法門之英雄無敵：王朝`,
`快打旋風：對決`,
`記憶事務所`,
`BLADE 刀鋒戰記 2`,
`影之刃：斷罪者`,
`影之刃||Phantom Blade || DARK BLADE`,
...
`暗黑破壞神 3`,
`三國 BLADE：再戰`,
`伊羅安||Elyon || A:IR(Ascent: Infinite Realm)`,
`怪獸超級聯賽`,
`Boxing Star`,
`來我家玩吧`,
`守望傳說 || 坎公騎冠劍||Guardian Tales`,
`月光雕刻師||The Legendary Moonlight Sculptor`,
`奧丁：神叛 || 奧丁：瓦爾哈拉之崛起||ODIN : Valhalla Rising`,
`永恆靈魂||Eversoul`,
`傳奇 4||MIR4`,
`Terra Fire`,
`Friends Shot`,
`All Star Smash`,
`上古世紀||アーキエイジ||ArcheAge`,
`LINE：哆啦 A 夢樂園||LINE：ドラえもんパーク`,
`Calice`,
