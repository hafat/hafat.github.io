...
`全民燒腦 || 大腦要壞掉了啦||脳トレパズルゲーム - ブレーン Go||Brain Go`,
`全民燒腦2 || 大腦要壞掉了啦2||脳トレパズルゲーム - ブレーン Go 2||Brain Go2`,
`全民燒腦3 || 大腦要壞掉了啦3||脳トレパズルゲーム - ブレーン Go 3||Brain Go3`,
`超元氣火柴人`,
`英靈召喚師||SoulFactor || Soul Fact || Soul Faction`,
`暗影騎士：元氣之魂||シャドウナイト：忍者格闘アクションゲーム||Shadow Knight Batman Ninja War`,
`召喚師時代 ||サマナーズの時代: ヒーローのアリーナ・AFK RPG||Summoners Era: Idle RPG Heroes`,
`鯊鯊進化論||サメサメ進化論||Fish Eater.IO`,
`九霄神域`,
`不朽戰紀||聖剣戦記||Immortal Raid`,
`最強巫師||スーパーウィザード||Super Wizard`,
`明日方舟：終末地`,
`豬遊記||Piggy GO `,
`龍域：不滅勇者 || 龍域冒險：奧德賽`,
`創世尋仙`,
...
`最強海王||FIshpond War`,
`Fidget Play`,
`山海～目が覚めたら妖獣が女の子に!? || 山海～神話の世界を覗く異界万華鏡～`,
`混沌破壞者||カオスブレイカー||Chaos Breaker`,
`再生之境 || 少女戰姬||Clash of Sky`,
`萌娘戰姬-放置系三國女神物語||乙女戦姫～ロリータガールズ～`,
`慶餘年`,
`Apex 英雄 Mobile||Apex Legends Mobile`,
`庫洛蘭德 || 代號：放置RPG||Couroland`,
`七星傳||セブンスフィア||Seven Sphere`,
`虛國：元靈召喚 || 虛互之國`,
`Yeager`,
`煉金術士花園||アルケミストガーデン||Alchemists' Garden`,
` Yggdra Resonance||ユグドラ・レゾナンス`,
`新小森生活`,
`江湖大亨 || 桃園小鎮`,
`三分武俠七分仙 || 上仙請留步`,
`進擊的火柴人||進撃のマッチスティックメン||My Stick Life `,
`數碼異次元 || 數碼世界||Digital Master`,
`幻靈物語：塔之靈 || 長安幻靈錄 || 侍神誅妖||phantom story   || Spectral Tale`,
`文明與征服||Civilization and Conquest`,
`重生邊緣||SYNCED: Off-Planet`,
`戰鎚2：末日鼠疫||Warhammer: Vermintide 2`,
`戰鎚40K：黑潮 ||  戰鎚40K：暗潮||Warhammer 40,000: Darktide`,
`吸血鬼：惡夜獵殺 血獵||Vampire The Masquerade: Bloodhunt`,
`GTFO`,
`重金屬：地獄歌手||Metal: Hellsinger`,
`Awaken: Chaos Era`,
`King's Choice`,
`Armored God`,
`合成神奇島||Merge County`,
...
`Enigmite's Prophecy`,
`Catwalk Beauty`,
`籃球全明星||AllStars Basketball `,
`戰國大戰 M`,
`東方 Arcadia Record||東方アルカディアレコード`,
`Yulgang Global`,
`智城戰紀||パズル＆コンクエスト||Puzzles & Conquest`,
`First Refuge: Z`,
`時空之弈||Chrono Legacy`,
`蟻族奇兵 AntWars`,
`轉生史萊姆：魔物之王`,
`Z日英雄之心||Zデー: 戦争ヒーローの戦国対戦バトルキングダム||Z Day: Hearts of Heroes`,
`火器時代||ガンズ・オブ・グローリー：鉄仮面||Guns of Glory: The Iron Mask`,
`MÄRCHEN of LIGHT||メルヘン・オブ・ライト～モロガミ放置RPG～`,
`開心莊園||にじいろ牧場||Family Farm Seaside`,
`夢幻龍樂園||ドラゴンスケープスの冒険||Dragonscapes Adventure`,
`超級快遞大亨——3D商業管理遊戲||放置宅配便タイクーン - 3Dビジネスマネージャー||Idle Courier`,
`夢境偵探||ドリーム探偵||Dream Detective`,
`Tile Master 3D||ブロックマスター３D`,
`Cube Master 3D||キューブマスター3D`,
`天天愛連消||ブロークペア||Tile Connect - Classic Match`,
`小鎮大廚||シェフマージ||Chef Merge`,
`Higgs Domino Island`,
`豪神娛樂城`,
`我的學妹不可能那麼萌2||ガールズ X バトル 2||Girls X Battle 2`,
`黑道風雲||マフィア・シティ-極道風雲||Mafia City`,
`Infinite Galaxy`,
`Zgirls3`,
`成語消消挑戰`,
`成語填填字`,
...
`Pasha Fencer`,
`Raider: Origin`,
`Yong Heroes||剣魂～新ジョブ「悟空」降臨`,
`Kung Fu Legend`,
`浣熊不高興||Unhappy Raccoon`,
`伊甸騎士團 || 異世代:光與魔法 || 風之幻想：聖戰史歌||Knight Of The Eden`,
`Tik Dog`,
`Draw 2 Beat`,
`瘋小姐療養記||Happy Healer`,
`Tower Wars: Castle Battle||タワーバトル`,
`遊樂園在逃大亨||Jumbo Park`,
`Marie's Travel: Hidden objects||マリーの旅行：隠されたアイテム`,
...
`Magnum Quest`,
`逆空使命-NOVA計劃`,
`王朝起源征服||Dynasty Origins: Conquest`,
`王朝卷軸||Dynasty Origins: Pioneer || Dynasty Scrolls`,
`西遊戰棋||West Journey War`,
`我帶兵最牛`,
`Dark Nemesis: Infinite Quest`,
`Land of Empires: Immortal`,
`Rise of the Kings||ライズオブキングス`,
`War Eternal - Enter the Dragon`,
`Frontier Justice - Return to the Wild West||フロンティア・ジャスティス ワイルドな西部に戻る`,
`Revenge of Sultans`,
`魔法英雄||マジックラッシュ||Magic Rush: Heroes`,
`這就是江湖||侠客の天下||WuXia World `,
`我舔得賊溜||Licking Run`,
`足球冲冲冲||Football Run: Racing`,
`雪球快跑||Snowball Run`,
`節奏沙盒 || 節奏貓咪||Beat Cat||Musicat`,
`我的奇妙沙盒||サイキックのほこり||Psychic Dust - Pixel Sandbox`,
`我房子超大||SimHouse: Makeover`,
`飛刀彈彈彈||Draw Arrow`,
`瘋狂挖礦||Dig Crossing`,
`飛車大亂鬥-輕量級戰車競技手游||Crazy Car`,
`糖果大師||Candy Master 3D`,
`救救美人魚||Mermaid Rescue-Pull Her Out`,
`大冒險-馬丁歷險記||My Adventure-Martin Mystere`,
`腦洞先生||腦洞大作戰||Brain Hole Battle- brain game`,
`瘋狂搬磚||Idle Building`,
`旋轉音律||ロテーノ||Rotaeno`,
`神諭大陸 || 神領詩章||ジェネシスノポエム||The Ode to Greece`,
`航海王：夢想指針`,
`DC 英雄：放置聯盟||DC Worlds Collide`,
`幽遊白書：魔強統一戰`,
`火影忍者:巔峰對決||Naruto:SlugfestX`,
`新血魔伊甸園`,
`魔法禁書目錄 幻想收束`,
`雙生幻想||Azure Fantasy`,
`動物醫院||ハッピーどうぶつ病院||Happy Animal Hospital`,
`夢職人與追憶之黑妖精||夢職人と忘れじの黒い妖精||Dream Meister and the Recollected Black Fairy`,
`Art of War： Legions`,
`帝國大海戰||戦艦ファイナル||Battle Warship: Naval Empire`,
`熹妃Ｑ傳|| 謀りの姫：Pocket`,
`Apex 英雄手機版`,
`神廟逃亡2`,
`無限回戰||無限廻戦 - ローグライクSRPG||Re:Infinite - Roguelike SRPG`,
`魔力寶貝：旅人`,
`秘館疑踪||Heaven Dust`,
`花亦山心之月`,
`四葉草劇場||クローバーシアター`,
`勝利之吻||美女と栄光～戦場にキスの花束を～||Kiss of War`,
`天地の如く～激乱の三国志～`,
`三国覇王戦記～乱世の系譜～`,
`THE KING OF FIGHTERS '98UM OL`,
`MU：暗黑大天使||MU：アークエンジェル`,
`June's Journey ‑ Hidden Object||ミステリー探偵ジューン- 隠れた手がかり`,
`海戰傳奇 - Navy 1942||【戦艦】Warship Saga ウォーシップサーガ||Warship Saga`,
`覇王の天下`,
`逐鹿三國||三国天武～本格戦略バトル～ 三国志ストラテジー||Age of Three Kingdoms`,
`ラピスクロニクル -英雄王冠-||Lapis Chronicle -Hero Crown-`,
`少年的人間奇遇 || 宅男的人間冒險||オタクの人間冒険||Otaku's Adventure`,
`東方平野孤鴻||Ballads of Hongye`,
`劍訣天下`,
`葬花`,
`永劫無間||Naraka: Bladepoint`,
`山海萬花集 || 河圖尋仙記||Heroes of Mythic Might`,
`熱血江湖 W || 열혈강호 W`,
`守望傳說》日版`,
`仙劍客棧2`,
`冰封龐克 Mobile||Frostpunk: Rise of City || Frostpunk : Beyond the Ice`,
`蒼空物語 || 창공아레나 || 蒼空競技場||蒼空アリーナ||Blue Sky Arena || Changgong Arena`,
`庫洛魔法使：回憶鑰匙`,
`Front Mission：Borderscape`,
`銀河境界線`,
`新星：鋼鐵銀河||新星：Iron Galaxy||Nova: Iron Galaxy`,
`Shop Heroes Legends`,
`黑街異事||アーケイン・レイン||Arcane Lane`,
`MU ORIGIN 3`,
`火炬之光：無限`,
`Front Mission`,
`香腸派對||Sausage Man`,
`天龍八部 2||The Demi Gods & Semi Devils`,
`逍遙歌行`,
`泰拉瑞亞`,
`月之國度 || 星月命運之輪||Luna Online M || Star Moon Wheel of Fortune`,
`Merge Skyland`,
`動物朋友：王國`,
`新劍俠世界3 ||劍俠世界3`,
`節奏快打||Rhythm Fighter`,
