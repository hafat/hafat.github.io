...
`放置少女團 || 純白地平線 || 纯白境界 ||pure white realm || pure white horizon`,
`放置神域||Olympus: Idle Legends`,
`光之守望||Sins Raid: Heroes of Light`,
`櫻都學園 || 零界之都||Zero Kyokai`,
`魔天記3D ||魔天記 || 魔天M||Magic Heaven 3D`,
`將膽||Three Kingdoms: Heroes & Glory`,
...
`逆光潛入`,
`蒼穹幻夢`,
`Storia -寶物娘-||Storia - 宝物娘 -`,
`龍與世界的盡頭||Dragon and the end of the world`,
`數碼寶貝：新世紀`,
`Boom Beach: Frontlines`,
`王者榮耀‧世界`,
`奇蹟 MU：正宗續作`,
`SNK - 鬥魂集結 || 全明星激鬥||SNK All Stars Fight || All Stars Fight`,
`砰砰法師||PunBall`,
`RuneScape`,
`華夏群俠傳`,
`異世界的城主大人||Lord of the Other World`,
`全明星覺醒`,
`烏冬的旅店||Resortopia`,
`開心鬥一番`,
`龍與勇士`,
...
`豆腐女孩`,
`閃亂神樂 NEW LINK`,
`聯盟外傳：海克斯失序||Hextech Mayhem: A League of Legends Story`,
`匠木`,
`復甦的魔女||Revived Witch`,
`發現者||STAR STRIDE`,
`星遇online`,
...
`未來職業棒球||未来プロ野球`,
`女皇陛下 || 女皇物語 || 全民養成之女皇陛下`,
`永恆喚靈 || 魔法領主-放置戰爭||Immortal Summoners - Awaken`,
`惡魔覺醒 - 永恆之戰||Awaken Demons - Eternal war`,
`帕尼尼豪門足球`,
`亂世神將`,
`幽行玄城`,
`伊甸啟示錄`,
...
`聖樹喚歌||Holy Tree Song`,
`迷失蔚藍||LOST in Blue`,
`鐵匠傭兵團||Weapon Heroes : Infinity Forge`,
`島上驅魔人||Exorcist In Island`,
`小怪獸下樓梯||Little Monsters Down!`,
`跳躍吧小怪獸!||Jump! Little Monster!`,
`Fate/EXTELLA LINK||フェイト/エクステラ リンク`,
`DC Worlds Collide`,
`貓娘樂園來店||ネコぱらいてん！`,
`黎明啟示錄||Mobile Legends:Adventure`,
`姬械聯盟：藍星危機||armed girls union`,
`命運因子||Fading City`,
`乾坤飛劍傳 || 仙界龍俠傳`,
`新天龍八部 Online`,
`文明霸業-王者歸來||エボニー - 王の帰還||Evony-The King’s Return`,
`胖兔文明||Rabbit Empire`,
`影業大亨 `,
`昭和米國物語||Showa American Story`,
`放置魔法學院||Idle Magic School`,
`商業街||Shopping Mall Tycoon`,
`造個船吧||Arkcraft - Idle Adventure`,
`開心水族箱||Happy Fish: Dream Aquarium || My Fishbowl`,
`開心消消樂||Anipop`,
`海濱消消樂||Summer Pop`,
`幻劍九歌`,
`洛克王國||Roco Kingdom`,
...
`戰策三國志`,
`新墨魂`,
`寒冬計畫 Mobile||Project Winter Mobile`,
`阿凡達：復仇||Avatar：Reckoning`,
`我在長安有條街`,
`Bingo Aloha`,
`風之起源||Origin of Wind`,
`新信長之野望||新信長の野望`,
`葬花·暗黑桃花源||Funeral Flower·Dark Hei Peach Garden`,
`少女戰爭：幻想天下統一戰||少女ウォーズ: 幻想天下統一戦||Girl Wars: Fantasy World Unification Battle`,
`雄才三国～雄才で争う激動の天下～`,
`貓友圈 ~貓咪的旅行~`,
`麻雀一番街||Riichi City`,
`MetaCity M`,
`阿爾米婭讚歌 || 阿爾米亞讚歌 ||Almiya Anthem`,
`你已經猜到結局了嗎`,
`貪吃蛇進化論||Merge Snake!`,
`閒置龍塔 || 空閒女獵手:龍界||Idle Huntress: Dragon Realm`,
`Cash Frenzy`,
`維加斯賭場老虎機||Lotsa Slots™ - Vegas Casino`,
`Yalla Ludo`,
`蘇丹的遊戲||Game of Sultans`,
`西部風雲||West Game`,
`Be The King`,
`大福娛樂城||Jackpot World`,
`貓咪大迷宮`,
`苦手傳奇||異界之上||異世界苦手物語||Nigate Tale`,
`王國衛隊||Kingdom Guard: Tower Defense`,
`貪吃蛇大作戰2||Snake Off 2`,
`WePlay - 線上桌遊吧||WePlay - Party Game & Chat`,
`可汗的遊戲||Game of Khans`,
`我叫 MT：經典再現`,
`白荊迴廊`,
`仙夢如歌`,
`探險射射射`,
`蒼空物語 || 창공아레나||Blue Sky Arena || Changgong Arena`,
`以閃亮之名||Glamorous Secrets`,
`DK Mobile`,
`愛琳詩篇`,
`不朽覺醒`,
`Wonder Cash Casino`,
`鐵血江湖路`,
`賽馬娘||Uma Musume Pretty Derby`,
`波紋騎士`,
`萃星物語`,
`迷宮大偵探`,
`奧奇傳說`,
...
`Royal Shooter`,
`Hytale`,
`愛麗絲的衣櫥||アリスクローゼット||Alice Closet`,
`魔戒：開戰時刻||ロード・オブ・ザ・リング：戦いの幕開け||The Lord of the Rings：Rise to War`,
`我當校長超勇的`,
`仙靈奇緣`,
`雲山之約`,
`Cytoid`,
...
`Sparklite`,
`卡拉邦||CARAVAN STORIES`,
`天堂W||リネージュW||Lineage W`,
`Friends Racing`,
`女神指令`,
`英雄而已||Every Hero - Ultimate Action`,
`Rocket League`,
`火箭聯盟`,
`Mu Origin 3`,
`天子傳奇S`,
